#!/usr/bin/env python3
"""
Benchmark: Schnakenberg Reaction-Diffusion (Turing Morphogenesis)

System:
    du/dt = Du * lap(u) + gamma * (a - u + u^2*v)
    dv/dt = Dv * lap(v) + gamma * (b - u^2*v)

Parameters: a=0.1, b=0.9, gamma=1000, Du=1, Dv=40 (stripe regime)
Grid: 256x256, domain [0, 1]^2, t in [0, 100]
...
This problem is STIFFER than Gray-Scott: gamma=1000 drives reaction eigenvalues
...
print("Schnakenberg Benchmark")
print(f"Grid: {N}x{N}, Du={Du}, Dv={Dv}, a={a}, b={b}, gamma={gamma}")
...
# Initial condition: small random perturbation from homogeneous steady state
# Steady state: u_s = a + b, v_s = b / (a + b)^2
u_s = a + b
v_s = b / (u_s**2)
np.random.seed(42)
...
# Stability: dt < 2.8 / (D_max * lambda_max) for diffusion
# Also need dt < 1 / gamma for reaction stability
lambda_max = np.max(-lap_eig)
dt_diff = 2.8 / (max(Du, Dv) * lambda_max)
dt_react = 1.0 / gamma  # Reaction stiffness limit
...
print(f"   Full steps needed: {n_steps_rk4_full:,} (infeasible for direct run)")

# Run a SHORT timing burst of N_SHORT steps, extrapolate to full time
N_SHORT = 1000  # Enough for stable per-step timing

@jax.jit
def rk4_rhs_snak(u, v):
    ...
    u2v = u * u * v
    du = Du * lap_u + gamma * (a - u + u2v)
    dv = Dv * lap_v + gamma * (b - u2v)
    return du, dv

@jax.jit
def rk4_step_snak(u, v, dt):
    k1u, k1v = rk4_rhs_snak(u, v)
    k2u, k2v = rk4_rhs_snak(u + 0.5*dt*k1u, v + 0.5*dt*k1v)
    k3u, k3v = rk4_rhs_snak(u + 0.5*dt*k2u, v + 0.5*dt*k2v)
    k4u, k4v = rk4_rhs_snak(u + dt*k3u, v + dt*k3v)
...
dt_cn = 0.01  # Moderate dt — reaction stiffness limits CN too
...
@jax.jit
def cn_newton_step_snak(u_old, v_old):
    def compute_residual(u_new, v_new):
        ...
        R_u_new = gamma * (a - u_new + u2v_new)
        R_v_new = gamma * (b - u2v_new)
        R_u_old = gamma * (a - u_old + u2v_old)
        R_v_old = gamma * (b - u2v_old)
...
@jax.jit
def cn_solve_snak(u0, v0, n_steps):
    def body(i, uv):
        u, v = uv
        return cn_newton_step_snak(u, v)
    return jax.lax.fori_loop(0, n_steps, body, (u0, v0))

# Warmup
_ = cn_solve_snak(u0_jax, v0_jax, 10)
u0_jax.block_until_ready()

times_cn = []
for _ in range(N_REPS):
    start = time.perf_counter()
    u_cn, v_cn = cn_solve_snak(u0_jax, v0_jax, n_steps_cn)
...
if not bool(jnp.all(jnp.isfinite(u_cn))) or not bool(jnp.all(jnp.isfinite(v_cn))):
    print("   WARNING: CN solution contains NaN/Inf! Timing is invalid.")
results['methods'].append({
    ...
    'finite': bool(jnp.all(jnp.isfinite(u_cn))),
})
...
dt_imex = 0.001  # Reaction stability: |lambda_rxn| ~ 1000, need dt*|lambda|/2 < 1
n_steps_imex = int(np.ceil(T_FINAL / dt_imex))
t_actual_imex = n_steps_imex * dt_imex
print(f"   Steps: {n_steps_imex:,}, dt={dt_imex}")

@jax.jit
def imex_step_snak(u, v, dt):
    # Half step: explicit reaction
    u2v = u * u * v
    u = u + 0.5*dt * gamma * (a - u + u2v)
    v = v + 0.5*dt * gamma * (b - u2v)
...
if not bool(jnp.all(jnp.isfinite(u_imex))) or not bool(jnp.all(jnp.isfinite(v_imex))):
    print("   WARNING: IMEX solution contains NaN/Inf! Timing is invalid.")
results['methods'].append({
    'name': 'IMEX-Strang',
    ...
    'finite': bool(jnp.all(jnp.isfinite(u_imex))),
})
...
dt_etd = 0.001  # Reaction eigenvalues ~1000 limit explicit treatment: dt <= ~0.002
...
if not bool(jnp.all(jnp.isfinite(u_etd))) or not bool(jnp.all(jnp.isfinite(v_etd))):
    print("   WARNING: ETDRK4 solution contains NaN/Inf! Timing is invalid.")
results['methods'].append({
    'name': 'ETDRK4',
    ...
    'finite': bool(jnp.all(jnp.isfinite(u_etd))),
})
...
output_path = Path(__file__).parent / 'results' / 'schnakenberg.json'
...
np.save(output_path.parent / 'schnakenberg_u_final_etd.npy', np.array(u_etd))
np.save(output_path.parent / 'schnakenberg_v_final_etd.npy', np.array(v_etd))

print("\n" + "=" * 80)
print(f"SUMMARY - Schnakenberg, {dtype_str}, {device}")
print("=" * 80)
